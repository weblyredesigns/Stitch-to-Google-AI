...
import { MOCK_CAMPS } from '../constants';
...
const Camps: React.FC = () => {
  ...
  const [registrations, setRegistrations] = useState<Record<string, string[]>>({}); // { campId: [userIds] }
  ...
  useEffect(() => {
    const loadData = () => {
      ...
      const userCamps = JSON.parse(localStorage.getItem('indiaBloodConnect_all_camps') || '[]');
      // Merge mock and user-created camps
      setAllCamps([...MOCK_CAMPS, ...userCamps]);

      const savedRegs = JSON.parse(localStorage.getItem('indiaBloodConnect_camp_registrations') || '{}');
  ...
  const handleRegister = (campId: string) => {
    ...
    localStorage.setItem('indiaBloodConnect_camp_registrations', JSON.stringify(newRegs));
  ...
  const getRegisteredCount = (camp: DonationCamp) => {
    const fromStorage = registrations[camp.id]?.length || 0;
    return camp.registeredCount + fromStorage;
  };
          ...
          <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-8">
            ...
            <div className="w-full lg:w-auto bg-slate-50 dark:bg-slate-800/50 p-6 rounded-3xl border border-slate-100 dark:border-slate-800">
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                <div className="space-y-1">
                  ...
                  <select 
                    ...
                    className="w-full bg-white dark:bg-slate-900 border-slate-200 dark:border-slate-700 rounded-xl h-12 px-3 text-sm font-bold outline-none focus:ring-2 focus:ring-primary/20"
                  >
                ...
                <div className="space-y-1">
                  ...
                  <select 
                    ...
                    className="w-full bg-white dark:bg-slate-900 border-slate-200 dark:border-slate-700 rounded-xl h-12 px-3 text-sm font-bold outline-none focus:ring-2 focus:ring-primary/20 disabled:opacity-50"
                  >
                ...
                <div className="space-y-1">
                  ...
                  <div className="relative">
                    <input 
                      ...
                      className="w-full bg-white dark:bg-slate-900 border-slate-200 dark:border-slate-700 rounded-xl h-12 px-3 pr-10 text-sm font-bold outline-none focus:ring-2 focus:ring-primary/20"
                    />
              ...
              <div className="mt-4 flex justify-end">
                <button 
                  onClick={() => { setSelectedState(''); setSelectedDistrict(''); setSearchCity(''); }}
      ...
      <main className="py-16 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 text-left">
          {filteredCamps.map((camp) => {
            ...
            const isOrganizer = currentUser && camp.organizerId === currentUser.id;
            ...
            return (
              <div key={camp.id} className="bg-white dark:bg-slate-900 rounded-[2rem] overflow-hidden shadow-sm border border-slate-100 dark:border-slate-800 hover:shadow-xl transition-all flex flex-col md:flex-row">
                <div className="relative w-full md:w-64 h-48 md:h-auto">
                  <img alt={camp.name} className="w-full h-full object-cover" src={camp.imageUrl} />
                  <div className="absolute top-4 left-4 bg-white/90 dark:bg-slate-900/90 backdrop-blur-md px-3 py-1.5 rounded-lg text-[10px] font-bold text-primary shadow-sm uppercase tracking-wider">
                    {camp.tag}
                  </div>
                  {isOrganizer && (
                    <div className="absolute bottom-4 left-4 bg-blue-600 text-white px-3 py-1.5 rounded-lg text-[10px] font-black uppercase tracking-widest shadow-lg">
                      Hosted by You
                    </div>
                  )}
                </div>
                <div className="p-8 flex-1 flex flex-col justify-between">
                  <div>
                    ...
                    <div className="space-y-2 mb-6 text-slate-600 dark:text-slate-400">
                  ...
                  <div className="flex items-center justify-between pt-6 border-t border-slate-50 dark:border-slate-800">
                    <div className="flex items-center gap-2">
                      <div className="w-10 h-10 rounded-2xl bg-slate-100 dark:bg-slate-800 flex items-center justify-center text-xs font-black text-slate-700 dark:text-slate-300">
                        +{getRegisteredCount(camp)}
                      </div>
                    ...
                    {isUserRegistered ? (
                      ...
                      <button onClick={() => navigate('/dashboard')} className="bg-slate-100 text-slate-600 px-6 py-3 rounded-xl text-xs font-black hover:bg-slate-200 transition-all">
                        Manage Camp
                      </button>
                    ) : (
                      <button 
                        onClick={() => handleRegister(camp.id)}
                        className="bg-primary text-white px-8 py-3 rounded-xl text-xs font-black hover:bg-red-700 transition-all active:scale-95 shadow-lg shadow-primary/20"
                      >
                        Register Now
                      </button>
                    )}
        ...
        {filteredCamps.length === 0 && (
          <div className="py-20 text-center bg-white dark:bg-slate-900 rounded-[3rem] border-2 border-dashed border-slate-200 dark:border-slate-800">
             ...
             <button 
                onClick={() => { setSelectedState(''); setSelectedDistrict(''); setSearchCity(''); }}
                className="mt-6 px-6 py-2 bg-primary text-white rounded-xl font-bold text-sm"
              >
                Clear All Filters
              </button>
          </div>
